...
3. Apps → Update Apps List → install **Laudie — AI Product Image Enrichment**.
...
| Model | Purpose |
|---|---|
| `laudie.enrichment.job` | Batch job, processed 5 products / cron tick |
| `laudie.product.image.candidate` | AI-discovered image awaiting review |
| `laudie.enrichment.log` | Per-step audit trail |
| `laudie.ai.usage.log` | Anthropic API usage + cost |
| `laudie.scraping.recipe` | Per-domain learned CSS selectors (self-built after 5 AI successes) |

## XML-RPC API

```python
models.execute_kw(db, uid, pw, 'laudie.enrichment.job', 'laudie_enrich_by_skus',
                  [['SKU-1', 'SKU-2'], 'discover_only'])
models.execute_kw(db, uid, pw, 'laudie.enrichment.job',
                  'laudie_normalize_existing_images', [None])  # all
```
