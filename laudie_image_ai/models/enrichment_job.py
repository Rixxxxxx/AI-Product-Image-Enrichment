...
class EnrichmentJob(models.Model):
    ...
    pending_product_ids = fields.Many2many(
        'product.template', 'laudie_job_pending_rel', 'job_id', 'product_id',
        copy=False,
    )
    ...
    log_ids = fields.One2many('laudie.enrichment.log', 'job_id')
    candidate_ids = fields.One2many('laudie.product.image.candidate', 'job_id')
    ...
    def _cron_run_jobs(self):
        ...
        if not got_lock:
            _logger.info('laudie_image_ai cron: another worker holds the xact lock, skipping this tick.')
            return
    ...
    def _process_chunk(self):
        ...
        for product in chunk:
            try:
                ...
                try:
                    product.write({
                        'laudie_enrichment_state': 'error',
                        'laudie_enrichment_error': str(e)[:5000],
                    })
                except Exception:
                    pass
                failure += 1
    ...
    def _check_budget_and_pause(self):
        ...
        if not budget:
            return
        spent = self.env['laudie.ai.usage.log']._month_to_date_cost()
        if spent >= budget:
    ...
    def _notify_budget_warning(self, spent, budget):
        ...
        self.env['mail.mail'].sudo().create({
            'subject': f'[Laudie AI] Budget at 80% (${spent:.2f}/${budget:.2f})',
    ...
    def _notify_budget_exhausted(self, spent, budget):
        ...
        if not email:
            return
        self.env['mail.mail'].sudo().create({
            'subject': f'[Laudie AI] BUDGET EXHAUSTED — jobs paused (${spent:.2f}/${budget:.2f})',
    ...
    @api.model
    def laudie_enrich_by_skus(self, skus, pipeline='discover_only'):
    ...
    @api.model
    def laudie_normalize_existing_images(self, product_ids=None):
    ...
    @api.model
    def laudie_get_pending_candidates_for_product(self, product_id):
        cands = self.env['laudie.product.image.candidate'].search([
